...
```
Devices > Windows > Windows enrolment → Select Enrolment Status Page
```
...
![ESP Create Profile](../images/esp-create-profile.png)
...
![ESP Settings](../images/esp-settings.png)
...
![Enrolment Overview](../images/enrollment-overview.png)
...
![Deployment Profile Basics](../images/deployment-profile-basics.png)

#### Out-of-Box Experience (OOBE) Settings

| Setting | Value | Notes |
|---|---|---|
| Deployment mode | Self-Deploying | Minimizes user interaction; only input may be username/password if required |
| Join to Microsoft Entra ID as | Microsoft Entra joined | Greyed out in self-deploying mode — handled automatically |
| Microsoft Software License Terms | Hide | Streamlines the OOBE experience |
| Privacy settings | Hide | Reduces prompts during setup |
| Hide change account options | Hide | Prevents users from changing account settings |
| User account type | Standard | Best practice for security; avoid granting local admin |
| Language (Region) | Operating system default | Supports users across different countries with localised preferences |
| Automatically configure keyboard | Yes | Matches keyboard to the region automatically |
| Apply device name template | Yes | Maintains a consistent naming convention across the organisation |

![OOBE Settings](../images/oobe-settings.png)
...
![Device Enrolled](../images/device-enrolled.png)

#### Assign and Review

Assign the deployment profile to the group **"Sec - All Laptop/Desktops"**.

![Deployment Assignment](../images/deployment-assignment.png)

Review and create.

![Deployment Review](../images/deployment-review.png)
...
- **Individual devices:** You'll need to extract and upload them manually using PowerShell.
...
- The dynamic group rule `(device.devicePhysicalIDs -any (_ -contains "[ZTDId]"))` may take up to 24 hours to fully populate after hardware hashes are imported
